namespace Terrasoft.Core.Process
{
	...
	#region Class: PvkCalcAverageTicketPriceProcessMethodsWrapper

	/// <exclude/>
	public class PvkCalcAverageTicketPriceProcessMethodsWrapper : ProcessModel
	{

		public PvkCalcAverageTicketPriceProcessMethodsWrapper(Process process)
			: base(process) {
		...
		private bool ScriptTask1Execute(ProcessExecutingContext context) {
			var esq = new EntitySchemaQuery(UserConnection.EntitySchemaManager, "PvkYacht");
			var passengersCountColumn = esq.AddColumn("PvkPassengerCount");  // SELECT UsrPassengersCount as UsrPassengersCount, UsrPrice as UsrPrice FROM UsrYacht WHERE ...
			var priceColumn = esq.AddColumn("PvkPriceperdayEUR");
			
			Guid driveTypeId = Get<Guid>("DriveTypeIdParameter");
			
			var typeFilter = esq.CreateFilterWithParameters(FilterComparisonType.Equal, "PvkDriveType", driveTypeId);
